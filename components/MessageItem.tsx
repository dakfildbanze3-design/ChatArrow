...
export const MessageItem: React.FC<MessageItemProps> = ({ message, settings }) => {
  ...
  if (isUser) {
    return (
      <div className="flex flex-col items-end w-full mb-8 animate-in slide-in-from-right-4 duration-300 px-2 md:px-0 group/msg">
        ...
        <div className="max-w-[90%] md:max-w-[75%] px-4 py-3 rounded-2xl leading-relaxed text-[16px] bg-zinc-100 dark:bg-zinc-800/80 text-zinc-900 dark:text-white rounded-tr-none shadow-sm">
  ...
  const markdownComponents = {
    h1: ({ children }: any) => <h2 className="text-[24px] font-bold text-zinc-900 dark:text-white mt-8 mb-4 tracking-tight">{children}</h2>,
    h2: ({ children }: any) => {
      ...
      return (
        <h2 className="text-[20px] font-bold text-zinc-900 dark:text-white mt-8 mb-4 tracking-tight flex items-center gap-3">
          {hasNumber && (
    ...
    p: ({ children }: any) => <p className="text-[16px] leading-[1.6] text-zinc-700 dark:text-zinc-300 mb-4 font-normal">{children}</p>,
    ...
    li: ({ children }: any) => (
      <li className="text-[16px] flex items-start gap-3">
